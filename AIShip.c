...
void CreateShipEnvironment()
{
	...
	if (CheckAttribute(pchar, "Ship.Voice.GunsNotReady.SoundID"))
    {
        pchar.Ship.Voice.GunsNotReady.SoundID = -1;
    }
}	
...
void Ship_DoFire()
{
    ...
    if (CrosshairHidden() && !bCameraOutside)
    {
        string sBort;
        float fAng;
        GetFireCameraBort(&sBort, &fAng);
    
        string sFullBortName = "cannon" + sBort;
        
        bool bReady = false;
        
        if (CheckAttribute(pchar, "Ship.Cannons.borts." + sFullBortName + ".ChargeRatio"))
        {
            float fChargeRatio = stf(pchar.Ship.Cannons.borts.(sFullBortName).ChargeRatio);
            if (fChargeRatio >= 1.0)
            {
                bReady = true;
            }
        ...
        if (!bReady)
        {
            ...
            if (CheckAttribute(pchar, "Ship.Voice.GunsNotReady.SoundID"))
            {
                iLastSoundID = sti(pchar.Ship.Voice.GunsNotReady.SoundID);
            }
            bool bIsPlaying = false;
            if (iLastSoundID != -1)
            {
                bIsPlaying = SendMessage(&Sound, "ll", MSG_SOUND_IS_PLAYING, iLastSoundID);
            }
            if (!bIsPlaying)
            {
                //log_testinfo("Орудия борта " + sFullBortName + " не готовы!");
                int iNewSoundID = PlaySound("interface\" + LanguageGetLanguage() + "\_NotReady.wav");
                pchar.Ship.Voice.GunsNotReady.SoundID = iNewSoundID;
            }
...
void Ship_BortReloadEvent()
{
  ...
  if (sti(aCharacter.Tmp.BortsReloaded.Event))
  {
    ...
    else
    {
      ...
      if (bBack)  { PlaySound("interface\" + LanguageGetLanguage() + "\_GunReadyB.wav"); }
    }
  }
...
{         // че-то распук метод "неподецки" - переделал 29.07.06 boal
    ...
    {  // переход на мин команду
        PlaySound("interface\\" + LanguageGetLanguage() + "\\_Damage1.wav");
    }
...
{
    // Если выстрел не нанёс урон врагу
    if(fHP <= 0.0)  // Нет урона = промах
    {
        iLastShotsMissed++;
        
        if(iLastShotsMissed >= 3)  // После 3 промахов подряд
        {
            PlaySound("interface\\" + LanguageGetLanguage() + "\\_GTTown0.wav");
            iLastShotsMissed = 0;  // Сброс счётчика
        }
    }
    else
    {
        iLastShotsMissed = 0;  // Попадание = сброс
    }
}
	...
	{ 
		if (rOurCharacter.id == pchar.id || rand(2) == 1)
		{
			...
			PlaySound("interface\\" + LanguageGetLanguage() + "\\_Damage0.wav");
		}
...
        	{
			PlaySound("interface\" + LanguageGetLanguage() + "\_Abandon" + (rand(3) + 1) +".wav");
			}
			// boal <--
